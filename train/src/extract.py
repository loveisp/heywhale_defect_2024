...
img_path = Path('./data/train/imgs')
img_path.mkdir(parents=True, exist_ok=True)
mask_path = Path('./data/train/masks')
mask_path.mkdir(parents=True, exist_ok=True)
csv_path = Path('./data/train/csv文件/')
...
img_path = Path('./data/test_A/imgs')
img_path.mkdir(parents=True, exist_ok=True)
mask_path = Path('./data/test_A/masks')
mask_path.mkdir(parents=True, exist_ok=True)
csv_path = Path('./data/test_A/csv文件/')
...
df = pd.read_csv('./data/test_A/提交样例.csv')
processes = 16
with Pool(processes) as p:
    list(tqdm(p.imap(_foo, df.fileName.values), total=len(df)))